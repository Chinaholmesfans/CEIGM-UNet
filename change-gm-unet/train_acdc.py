...
class ACDC(L.LightningModule):
    ...
    def prepare_data(self) -> None:
        ...
        self.train_dataset = ACDCDataset(
            base_dir="/root/autodl-tmp/edata/ACDC",
            split="train",
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if self.current_epoch <= 250:
            # 在 epoch <= 250 时禁用验证
            self.trainer.check_val_every_n_epoch = 20
        else:
            # 在 epoch > 250 时每个 epoch 验证一次
    ...
    def on_train_epoch_end(self) -> None:
        ...
        self.log_and_logger("current_epoch", self.current_epoch)
        self.tl_metric.reset()

        # 在 epoch >= 200 时，每个 epoch 手动进行一次验证
    ...
    def validation_step2(self, batch: dict[str, torch.Tensor]) -> None:
        volume, label = batch["image"], batch["label"]
        ...
        with torch.no_grad():
            prediction = self._model(volume)
            out = torch.argmax(torch.softmax(out, dim=1), dim=1).squeeze(0)
            dcs = dc(out.cpu().data.numpy(), batch.cpu().data.numpy())

        self._model.train()

        self.vs_metric["dice"]["mean"].append(dcs)
    ...
    def on_validation_epoch_end2(self) -> None:
        avg_metric = np.mean(self.vs_metric["dice"]["mean"])
        self.log(f"val_dice_mean", t)
        self.log_and_logger(f"val_mean_dice", avg_metric)
        self.vs_metric = defaultdict(lambda: defaultdict(list))
...
def train(name: str) -> None:
    ...
    logger.add(join(name, today + "training.log"))

    model = ACDC(name)

    checkpoint_callback = ModelCheckpoint(
        dirpath=join(name, today + "checkpoints"),
...
if __name__ == "__main__":
    L.seed_everything(1998)
    monai.utils.set_determinism(1998)
    train("/root/autodl-tmp/log/msvm-unet-acdc")
