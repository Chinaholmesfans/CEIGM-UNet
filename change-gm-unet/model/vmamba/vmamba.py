...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,
        ...
        # conv =======================================
        if self.with_dconv:
            self.d_try = kwargs.get("d_try") if kwargs.get("d_try") is not None else False
            if self.d_try:
                self.convd_try = InceptionDWConv2d31d(d_inner,[])
                self.conv2d = nn.Conv2d(
                in_channels=d_inner,
                out_channels=d_inner,
                groups=d_inner,
            ...
            if d_conv == 300:
                # self.conv2d = InceptionDWConv2d31d(d_inner,[])
                self.conv2d = InceptionDWConv2d_MultiScale(d_inner,[])
                # self.conv2d = InceptionDWConv2d31d(d_inner,[])
            else:
                self.conv2d = nn.Conv2d(
                in_channels=d_inner,
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if not self.d_try:
            ...
            if a1 is None or a2 is None:
                exit()
            out = self.dropout(self.out_proj(a1 * y) + a2)
            """
            # dw + silu + conv7
            pass
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.try_c = False
        if self.try_c:
            self.tt1 = InceptionDWConv2d31(hidden_dim,[])
            self.tt2 = nn.SiLU()
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.ssm_branch:
            if self.post_norm:
                ...
                if self.try_c:
                    after_ln = self.norm(x)
                    a2 = x
                    a1 = self.tt2(self.tt1(after_ln))
                    x = self.drop_path(self.op(after_ln,a1=a1,a2=a2))
                else:
...
# vssblock2 only /7.0 + SiLU

# =====================================================
class VSSBlock2(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
...
class InceptionDWConv2d311(nn.Module): # 92.77
    def __init__(self, in_features, kernel_sizes, square_kernel_size = 3, band_kernel_size = 11):
        super(InceptionDWConv2d31, self).__init__()
        self.in_features = in_features
        self.out_features = in_features
        
        gc = math.ceil(self.out_features / 8.0)
    ...
    def forward(self, x):
        ...
        elx, erx = torch.chunk(x, 2, dim=1)

        xid, dw, dhw, dh= torch.chunk(erx, 4, dim=1)

        dw = self.dwconv_w(dw)
        dhw = self.dwconv_hw(dhw)
        dh = self.dwconv_h(dh)

        x_cat = self.dx + torch.cat((elx,xid, dw, dhw, dh), dim=1)

        return x_cat

class InceptionDWConv2d31d(nn.Module): # 92.71
    def __init__(self, in_features, kernel_sizes, square_kernel_size = 3, band_kernel_size = 11):
        super(InceptionDWConv2d31d, self).__init__()
        self.in_features = in_features
        self.out_features = in_features
    ...
    def forward(self, x):
        ...
        x_cat = torch.cat((elx1, elx2, dw, dhw, dh, erx1,erx2), dim=1)
...
class InceptionDWConv2d_MultiScale(nn.Module): # 92.74
    def __init__(self, in_channels,kernel_sizes = [1,3,5]):
        ...
        self.gc = self.out_features // 8
        self.half = self.out_features // 2
        self.id = self.half - 3 * self.gc
    ...
    def forward(self, x):
        ...
        elx, x_id, x_3x3, x_5x5, x_7x7 = torch.split(self.dx,(self.half, self.id, self.gc,self.gc,self.gc),dim = 1)
        ...
        return self.dx + torch.cat((elx, x_id, out_3x3, out_5x5, out_7x7), dim=1)
