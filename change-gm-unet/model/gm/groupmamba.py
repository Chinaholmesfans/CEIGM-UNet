...
class Block_mamba(nn.Module):
    def __init__(self, 
        ...
        self.attn = GroupMambaLayer(dim, dim)
        ####
        self.mlp = PVT2FFN(in_features=dim, hidden_features=int(dim * mlp_ratio)) if custom_mlp is None else custom_mlp(in_features=dim, hidden_features=int(dim * mlp_ratio))
...
class GroupMamba(nn.Module):
    ...
    def forward_features(self, x):
        ...
        """
            ...
            x = self.forward_cls(x, 1, 1)
            x1,y1 = x[:,:49,:],x[:,-1,:].unsqueeze(1)
            x = norm(x1 + y1)
            x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        """
