...
def get_model(ckpt: str, **kwargs: Any) -> nn.Module:
    ...
    for i in range(4):
        f1 = f"decoder.para{i+1}.channel_attention.conv2_1.weight"
        f2 = f"decoder.para{i+1}.channel_attention.conv2_2.weight"
        f3 = f"decoder.para{i+1}.channel_attention.conv3.weight"

        b,c,h,w = state_dict[f1].shape
        for aa in range(b):
            for bb in range(c):
                vs = state_dict[f1][aa][bb][h//2][w//2]
                for cc in range(h):
                    for dd in range(w):
                        state_dict[f1][aa][bb][cc][dd] = vs
        state_dict[f1] = global_avg_pool(state_dict[f1])

        b,c,h,w = state_dict[f2].shape
        for aa in range(b):
            for bb in range(c):
                vs = state_dict[f2][aa][bb][h//2][w//2]
                for cc in range(h):
                    for dd in range(w):
                        state_dict[f2][aa][bb][cc][dd] = vs
        state_dict[f2] = global_avg_pool(state_dict[f2])

        b,c,h,w = state_dict[f3].shape
        for aa in range(b):
            for bb in range(c):
                vs = state_dict[f3][aa][bb][h//2][w//2]
                for cc in range(h):
                    for dd in range(w):
                        state_dict[f3][aa][bb][cc][dd] = vs
        state_dict[f3] = global_avg_pool(state_dict[f3])
        '''
...
def test_acdc(ckpt: str) -> None:
    ...
    output_folder = "/root/autodl-tmp/test_acdc/img_out/0.8561_2"
    logger.add(os.path.join(output_folder, "test.log"))

    device = "cuda:0"
    model = get_model(ckpt=ckpt, in_channels=3, num_classes=4).to(device)
    dataset = ACDCDataset(base_dir="/root/autodl-tmp/edata/ACDC", split="test")
...
def test_synapse(ckpt: str) -> None:
    ...
    output_folder = "/root/autodl-tmp/test_synapse/img_out/0.8561_2"
    logger.add(os.path.join(output_folder, "test.log"))

    device = "cuda:0"
    model = get_model(ckpt=ckpt, in_channels=3, num_classes=9).to(device)
    dataset = SynapseDataset(base_dir="/root/autodl-tmp/edata/Synapse/test_vol_h5", split="test_vol")
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)
    inference(
        model=model,
        dataloader=dataloader,
...
if __name__ == '__main__':
    # test_acdc(ckpt="/root/autodl-tmp/log/msvm-unet-acdc/2025_3_27_01_checkpoints/epoch=289-val_mean_dice=0.9258.ckpt")
    # test_acdc(ckpt="/root/autodl-tmp/log/msvm-unet-acdc/2025_3_24_03_checkpoints/epoch=269-val_mean_dice=0.9281.ckpt")
    # test_synapse(ckpt="/root/autodl-tmp/log/gm-unet-synapse/2025_3_10_03_checkpoints/epoch=174-val_mean_dice=0.8590.ckpt")
    # test_acdc(ckpt="log/msvm-unet-acdc/checkpoints/epoch.219-val_mean_dice.0.9258.ckpt")
    # test_acdc(ckpt="/root/autodl-tmp/log/msvm-unet-acdc/2025_2_27_03_checkpoints/epoch=239-val_mean_dice=0.9279.ckpt")
    # test_synapse(ckpt="/root/autodl-tmp/log/epoch=229-val_mean_dice=0.8564.ckpt")
    test_synapse(ckpt="/root/autodl-tmp/log/gm-unet-synapse/2025_6_16_02_checkpoints/last.ckpt")
